...
outFile = open("Z:\Documents\Shirly\Ontology Research\Output\StreamFlowsIntoLakesPonds.txt", "w")
print "Workspace Set"
...
where_clause = "From_GNIS_Name <> ' ' AND To_GNIS_Name <> ' ' AND From_GNIS_Name <> To_GNIS_Name AND (From_FCODE = 46006 OR From_FCODE = 46003) AND ( To_FCODE = 56600)"
...
for rows in plusFlow_Results:
    ...
    To_COMID = rows.getValue("ToCOMID")
    ...
    for result_rows in output_Layer_Results:
        To_GNISID = result_rows.getValue("GNIS_ID")
        if To_GNISID.strip():
