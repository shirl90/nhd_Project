...
outFile = open("Z:\Documents\Shirly\Ontology Research\Data-To Work\StreamFlowsFromLakesPonds.txt", "w")
print "Workspace Set"
...
where_clause = "From_GNIS_Name <> ' ' AND To_GNIS_Name <> ' ' AND (From_FCODE = 55800) AND ( To_FCODE = 46006 OR From_FCODE = 46003)"
...
for rows in plusFlow_Results:
    ...
    From_COMID = rows.getValue("FROMCOMID")
    # Make a feature layer containing only the feature of interest
    arcpy.MakeFeatureLayer_management(nhdFlowLine,"SelectionLineLayer",'"' + str(nameField) + '" =' + str(From_COMID))
    ...
    for result_rows in output_Layer_Results:
        From_GNISID = result_rows.getValue("GNIS_ID")
        if From_GNISID.strip():
